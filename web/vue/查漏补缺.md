# Vue知识点
## v-for和v-if的优先级谁更高？
* 在Vue2.x里是 v-for 的优先级高于 v-if ，因此会出现 如果同时使用，即使很多元素不应该显示，但也会先被渲染出来，再隐藏
* 因此，在Vue3.x里，v-if 的优先级高于 v-for ，当然，官方也依旧不推荐这样使用！
...
* 适用于多个数据源变化时执行一些自定义逻辑，比如异步请求、动画效果等 或者 要将这些数据一起计算，或者 需要新旧值共同参与的逻辑。如果单个数据源的变化更推荐通过 监听事件的时候做，不推荐直接使用 watch。
* 需要定义处理函数，并在函数中执行相应的操作。

他们最大的区别是：Computed 是返回的一个值，而 Watch 是一个执行函数，不会返回值。

> [解锁Vue 3的神秘力量：深入理解computed和watch](https://juejin.cn/post/7308563909560549391?searchId=2024042923275154F8BD5001C1D83F98B7#heading-23)
...
1. 数据劫持：在 new Vue() 后， Vue 会调用 _init 函数进行初始化，也就是init 过程，在 这个过程Data通过 Observer类 将 data里的数据 转换成了getter/setter的形式，从而实现数据劫持。
2. 创建Dep依赖对象：在属性转化为响应式属性时，Vue 会为每个属性创建一个「Dep」（依赖）对象，这个对象可以用于收集当前属性的依赖关系
3. 收集依赖：当render函数 执行的时候，因为视图渲染需要显示对应的数据，因此会读取所需对象的值，这就意味着getter函数被触发，Dep将会对所有使用到变量的地方进行收集，将对应的Watcher加入到Dep中，从而完成了依赖收集。
4. 通知变更：在修改对象的值的时候，会触发对应的setter， setter通知通过依赖收集得到的 Dep 中的每一个 Watcher，告诉它们自己的值改变了，需要重新渲染视图。这时候这些 Watcher就会开始调用 update 来更新视图。
...
详细的推荐这两篇文章：
1. [「Vue源码学习(一)」你不知道的-数据响应式原理](https://juejin.cn/post/6968732684247892005)
2. [深入浅出Vue响应式原理（完整版）](https://juejin.cn/post/6844903882208837640?searchId=20240426131221099E953D7467AE7581C2)
3. [Vue的响应式数据与双向绑定：原理与实现解析](https://juejin.cn/post/7236593298763776060?searchId=202404260952192C950FD27880B15C47F2)
### nextTick 的原理是什么？
...
### pinia和vuex的区别
1. Pinia 是一个更轻量级的状态管理库，移除了 Mutation 和 Module，只提供了 State、Action 和 Getter。学习曲线更低。
2. Pinia 是一个支持多Store的扁平化结构，而 Vuex 是一个单Stroe 使用 Module 的嵌套树形结构。
```ts
...
```
3. Pinia 使用TypeScript开发，对TypeScript的语法支持更友好，同时 对Vue3 的 setup 语法更适配。
4. Pinia 支持SSR渲染，还支持 自定义插件进行额外的扩展。
5. Pinia 不支持时间旅行和编辑等调试功能。

> 用法：[一文解析Pinia和Vuex，带你全面理解这两个Vue状态管理模式](https://juejin.cn/post/7121209657678364685?searchId=202404281418285BEDDD715A251801A23D#heading-14)
...
先把 template 模板转化为 render函数，然后利用 render 函数，直接将 虚拟Dom 转换为 真实DOM
#### 组件化
> 组件化的渲染机制、keep-alive 以及 其他官方组件
### Composition API性能上和 Option API有什么区别吗？
...
### Keep-Alive的原理
每个 Vue 组件都有一个生命周期，包括创建、挂载、更新和销毁等阶段。当组件被销毁时，它的状态和 DOM 结构也会被销毁。

<keep-alive> 是一个内置抽象组件，其工作原理是将需要缓存的组件保存在内存中，而不是销毁它们。这样，当你再次切换到这些组件时，它们可以迅速恢复到之前的状态，而不需要重新创建和渲染。

它使用了 Vue 的抽象组件特性，通过 include 和 exclude 属性来控制哪些组件需要缓存。它还提供了一些生命周期钩子，以便你可以在组件被缓存或激活时执行特定操作。
...
#### 抽象组件
> [Vue@2.X --- 官方文档上没有的抽象（abstract）组件](https://juejin.cn/post/7221159922905464869?searchId=20240430102427E52ED2A1BD4E576E8E4F)
> [实现一个vue3组件库 - partial挑选(抽象组件)](https://juejin.cn/post/7278583672402935862?searchId=20240430102427E52ED2A1BD4E576E8E4F)

Vue.js提供了一个`abstract`开关，在组件配置项中添加一个`abstract`的选项设置为`true`，就可以让组件成为一个抽象组件，`抽象组件它自身不会渲染一个DOM元素（不会渲染标签出来），也不会出现在组件的父组件链中`（子组件里通过`this.$parent`获取不到）。

在抽象组件的生命周期过程中，我们可以对包裹的子组件监听的事件进行拦截，也可以对子组件进行Dom 操作，从而可以对我们需要的功能进行封装，而不需要关心子组件的具体实现。

**抽象组件是Vue中唯一一个不会正常渲染到页面，也没有template模板的组件**

### Vue3的Composition API比Option API性能上有提升吗？
> [都说Vue3跟Vue2比，性能优化很厉害！](https://juejin.cn/post/7294928126940512282?searchId=202404292202566538902AFADCBB35151D#heading-7)
...
* [手写一个简易vue响应式带你了解响应式原理](https://juejin.cn/post/6989106100582744072?searchId=202404260952192C950FD27880B15C47F2#heading-10)